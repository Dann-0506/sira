...
/**
 * Objeto de Acceso a Datos (DAO) para Calificaciones (Resultados).
 * Maneja la persistencia individual y masiva del desempeño de los alumnos.
 */
public class ResultadoDAO {
    ...
    private Resultado mapear(ResultSet rs) throws SQLException {
        ...
        try { 
            ...
            r.setPonderacion(rs.getBigDecimal("ponderacion")); 
    ...
    public List<Resultado> findByInscripcionYUnidad(int inscripcionId, int unidadId) throws SQLException {
        String sql = """
                SELECT r.*, ag.nombre AS actividad_nombre, ag.ponderacion AS ponderacion
                FROM resultado r
                JOIN actividad_grupo ag ON ag.id = r.actividad_grupo_id
                WHERE r.inscripcion_id = ? AND ag.unidad_id = ?
                ORDER BY ag.id
                """;
    ...
    public void guardar(int inscripcionId, int actividadGrupoId, BigDecimal calificacion) throws SQLException {
        String sql = """
    ...
    /**
     * Guarda un lote de resultados utilizando procesamiento por lotes (Batch) 
     * y manejo manual de transacciones.
     */
    public void guardarLote(List<Resultado> resultados) throws SQLException {
        try (Connection conn = DatabaseManagerUtil.getConnection()) {
            conn.setAutoCommit(false);
            try {
                guardarLoteEnConexion(conn, resultados);
                conn.commit();
    ...
    /**
     * Utilidad para reutilizar una conexión existente (por ejemplo, desde un Orquestador) 
     * para realizar una inserción masiva.
     */
    public void guardarLoteEnConexion(Connection conn, List<Resultado> resultados) throws SQLException {
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            for (Resultado r : resultados) {
                ...
                if (r.getCalificacion() != null) ps.setBigDecimal(3, r.getCalificacion());
                else ps.setNull(3, Types.NUMERIC);
                ps.addBatch();
            }
            ps.executeBatch();
